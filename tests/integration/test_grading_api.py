...
class TestGradingRun:
    ...
    async def test_grading_run_requires_auth(self, async_client):
        ...
        response = await async_client.post("/api/v1/grading/run", json={
            "task_ids": [1],
            "teacher_answers": {"1": "C"},
        })
...
# ================================================================
# 6.9: GET /grading/results/{batch_id}
# ================================================================
...
class TestGradingResults:
    ...
    async def test_grading_results_requires_auth(self, async_client):
        """未认证返回 401/403。"""
        response = await async_client.get("/api/v1/grading/results/test-batch-id")
        assert response.status_code in (401, 403)
    ...
    async def test_grading_results_ok(self, async_client, teacher_headers):
        ...
        response = await async_client.get(
            "/api/v1/grading/results/test-batch-id",
            headers=teacher_headers,
        )
        assert response.status_code == 200
        data = response.json()
