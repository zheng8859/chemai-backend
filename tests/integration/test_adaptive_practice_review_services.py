...
async def _create_school(db: AsyncSession) -> School:
...
async def _create_student(db: AsyncSession, **kwargs) -> Student:
    ...
    school = await _create_school(db)
    grade = Grade(name="高一", school_id=school.id)
    db.add(grade)
    await db.flush()
    class_ = ClassModel(name="测试班", grade_id=grade.id)
    db.add(class_)
    await db.flush()
...
async def _create_question(
    ...
    **kwargs,
    ...
    """创建测试题目。"""
    from app.core.enums import QuestionType, QuestionSource, Difficulty
    defaults = dict(
        content=content,
...
class TestAdaptivePracticeCreate:
    ...
    @pytest.mark.anyio
    async def test_create_practice_for_new_student(self, db_session):
    ...
    @pytest.mark.anyio
    async def test_create_practice_with_kp_override(self, db_session):
        """手动指定知识点覆盖。"""
        student = await _create_student(db_session)
    ...
    @pytest.mark.anyio
    async def test_create_practice_zpd_based_on_history(self, db_session):
        """有历史的学生的 ZPD 基于过往作答。"""
        student = await _create_student(db_session)
...
class TestAdaptivePracticeSubmit:
    ...
    @pytest.mark.anyio
    async def test_submit_all_correct(self, db_session):
        """全部答对 → score = total, accuracy = 1.0。"""
        student = await _create_student(db_session)
        q = await _create_question(db_session, content="氧化铜与盐酸反应？", answer="B")
    ...
    @pytest.mark.anyio
    async def test_submit_duplicate_raises_error(self, db_session):
        """重复提交抛出异常。"""
        student = await _create_student(db_session)
        q = await _create_question(db_session)
...
class TestAdaptivePracticeTasks:
    ...
    @pytest.mark.anyio
    async def test_returns_empty_for_new_student(self, db_session):
        """新学生无任务。"""
        student = await _create_student(db_session)
    ...
    @pytest.mark.anyio
    async def test_returns_pending_after_create(self, db_session):
        """创建练习后出现在 pending 中。"""
        student = await _create_student(db_session)
        await AdaptivePracticeService.create_practice(
            db_session, student.id, question_count=5
...
class TestAdaptivePracticeEffect:
    ...
    @pytest.mark.anyio
    async def test_less_than_two_sessions_returns_none_improvement(self, db_session):
    ...
    @pytest.mark.anyio
    async def test_two_sessions_with_improvement(self, db_session):
        """2 次练习对比 → 计算进步率（不同题目避免交叉污染）。"""
        student = await _create_student(db_session)
        q1 = await _create_question(db_session, content="题A", answer="A")
        q2 = await _create_question(db_session, content="题B", answer="B")
        q3 = await _create_question(db_session, content="题C", answer="C")
        q4 = await _create_question(db_session, content="题D", answer="D")
...
class TestReviewComplete:
    ...
    @pytest.mark.anyio
    async def test_complete_review_correct_advances(self, db_session):
        """答对 2 次连续触发升级。"""
        student = await _create_student(db_session)
        q = await _create_question(db_session)
    ...
    @pytest.mark.anyio
    async def test_complete_review_wrong_downgrades(self, db_session):
        """答错下降级。"""
        student = await _create_student(db_session)
        q = await _create_question(db_session)
...
class TestReviewSync:
    ...
    @pytest.mark.anyio
    async def test_sync_creates_new_tasks(self, db_session):
        """新错题创建 ReviewTask。"""
        student = await _create_student(db_session)
        q = await _create_question(db_session)
    ...
    @pytest.mark.anyio
    async def test_sync_skips_existing_pending(self, db_session):
    ...
    @pytest.mark.anyio
    async def test_sync_pulls_back_completed(self, db_session):
...
class TestReviewTraining:
    ...
    @pytest.mark.anyio
    async def test_create_training_returns_session(self, db_session):
        """创建训练会话返回题目列表。"""
        student = await _create_student(db_session)
        q = await _create_question(db_session)
    ...
    @pytest.mark.anyio
    async def test_submit_training_all_correct(self, db_session):
        """全部答对，建议积极。"""
        student = await _create_student(db_session)
    ...
    @pytest.mark.anyio
    async def test_submit_training_poor_performance(self, db_session):
        """全错，建议回归教材。"""
        student = await _create_student(db_session)
        q = await _create_question(db_session, answer="C")
...
class TestDailyPracticeScheduler:
    ...
    @pytest.mark.anyio
    async def test_assigns_practice_to_approved_student(self, db_session):
        """已审批学生获得每日练习。"""
        student = await _create_student(db_session)
...
class TestDailyPracticeNotify:
    ...
    @pytest.mark.anyio
    async def test_overdue_tasks_updated_and_notified(self, db_session):
